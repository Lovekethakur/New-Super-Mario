...
var peachUp = Decoration.extend({
	init: function(x, y, level) {
		this._super(x, y, level);
		this.setImage(images.peach, 51, 0);
	},
}, 'peach_up');
var peachDown = Decoration.extend({
	init: function(x, y, level) {
		this._super(x, y, level);
		this.setImage(images.peach, 51, 32);
	},
}, 'peach_down');
...
var flagUpLeft = Decoration.extend({
	init: function(x, y, level) {
		this._super(x, y, level);
		this.setImage(images.flag, 0, 0);
	},
}, 'flag_up_left');

var flagUpRight = Decoration.extend({
	init: function(x, y, level) {
		this._super(x, y, level);
		this.setImage(images.flag, 32, 0);
	},
}, 'flag_up_right');


var flagup = Decoration.extend({
	init: function(x, y, level) {
		this._super(x, y, level);
		this.setImage(images.flag, 64, 0);
	},
}, 'flag_up');

var flagdown = Decoration.extend({
	init: function(x, y, level) {
		this._super(x, y, level);
		this.setImage(images.flag, 64, 32);
	},
}, 'flag_down');


var flagmidLeft = Decoration.extend({
	init: function(x, y, level) {
		this._super(x, y, level);
		this.setImage(images.flag, 0, 32);
	},
}, 'flag_mid_left');


var flagmidRight = Decoration.extend({
	init: function(x, y, level) {
		this._super(x, y, level);
		this.setImage(images.flag, 32, 32);
	},
}, 'flag_mid_right');

var flagDownLeft = Decoration.extend({
	init: function(x, y, level) {
		this._super(x, y, level);
		this.setImage(images.flag, 0, 60);
	},
}, 'flag_down_left');

var flagDownRigth = Decoration.extend({
	init: function(x, y, level) {
		this._super(x, y, level);
		this.setImage(images.flag, 32, 60);
	},
}, 'flag_down_right');
...
var Star = ItemFigure.extend({
	...
	move: function() {
		if(this.active) {
			...
			this._super();
			if(this.vx===0){
				this.setVelocity(this.direction === directions.right ? -constants.star_vx : constants.star_vx , this.vy=20);
...
var greenMushroomBox = Item.extend({
	init: function(x, y, level) {
		this._super(x, y, true, level);
		// this.setImage(images.objects, 96, 33);
		this.mushroom = new greenMushroom(x, y, level);
		// this.setupFrames(8, 4, false);
	},
	activate: function(from) {
		if(!this.activated) {
			this.mushroom.release(green_mushroom_mode.green_mushroom);
			this.clearFrames();
			this.bounce();
			this.setImage(images.objects, 514, 194);
		}
...
var greenMushroom = ItemFigure.extend({
	init: function(x, y, level) {
		this._super(x, y, level);
		this.active = false;
		this.setSize(32, 32);
	...
	move: function() {
		if(this.active) {
			...
			if(this.mode === green_mushroom_mode.green_mushroom && this.vx === 0)
				this.setVelocity(this.direction === directions.right ? -constants.mushroom_v : constants.mushroom_v, this.vy);
		} else if(this.released) {
			this.released--;
			this.setPosition(this.x, this.y + 8);
			...
			if(!this.released) {
				...
				if(this.mode === green_mushroom_mode.green_mushroom)
					this.setVelocity(constants.mushroom_v, constants.gravity);
			}
		}
	},
...
var Mario = Hero.extend({
	init: function(x, y, level) {
		...
		this.fast = false;
		this.setLevelNo(this.level.id+1);
	},
	...
	setLevelNo: function(levelNo) {
		this.levelNo = levelNo;
		
		
				
		this.level.world.parent().children('#lvl').text(this.levelNo);
	},
	addLife: function() {
...
$(document).ready(function() {
	...
	$('.Paused').click(function(){
		...
		if(level.paused == true){
			level.playMusic('pause');
			level.start();
			$("#pauseIcon").removeClass("fa-regular fa-circle-play").addClass("fa-regular fa-circle-pause");
		}
		else{
			level.pauseAudio();
			level.playMusic('pause');
			level.pause();
			$("#pauseIcon").removeClass("fa-regular fa-circle-pause").addClass("fa-regular fa-circle-play");
		}
	});
	$('.btn1').show();
	$('.btn').hide();
